...
<div class="container">
    @if(!empty(Auth::user()))
        @if(Auth::user()->level == 'pemilik')
            <a href="{{url('pemilik/kos/'.Auth::user()->id)}}"><i style="font-size: 2.5rem" class="fa fa-arrow-left mt-5"></i></a>
        @else
            <a href="{{url('kos/show')}}"><i style="font-size: 2.5rem" class="fa fa-arrow-left mt-5"></i></a>
        @endif
    @else
        <a href="{{url('kos/show')}}"><i style="font-size: 2.5rem" class="fa fa-arrow-left mt-5"></i></a>
    @endif
      ...
      <div class="col-md-6">
        <div id="carouselExampleControls" class="carousel slide" data-ride="carousel">
            <div class="carousel-inner">
                @for($i=0;$i<7;$i++)
                    <div class="carousel-item {{$i === 0 ? "active":""}}"> <img class="d-block w-100" src={{asset("img/".$kos->foto)}} alt="slide {{$i}}"> </div>
                @endfor
            </div>
        ...
        <div class="col-md-6">
            <div class="row">
                <h4 class="text-left mt-3" style="font-family: 'Poppins', sans-serif;">{{$kos->nama}}</h4>
            </div>
    
            <div>
                <div class="price-capt mr-3 mt-2 mb-3" style="font-family: 'Lato', sans-serif;"><i class="fa fa-money-bill-wave ml-3 mr-3" style="color: green;"></i>Rp {{$kos->harga}}/</a><a style="font-family: 'Lato', sans-serif;">Bulan</div>
                <div class="text-capt-detail mr-3-3 mt-2 mb-3" style="font-family: 'Lato', sans-serif;" ><i class="fa fa-map ml-3 mr-3" style="color: green;"></i>{{$kos->lokasi}}</div>
            </div>
            <div class="rows">
                <h5 class="text-left mt-2 mt-4 mb-4" style="font-family: 'Poppins', sans-serif;">Fasilitas</h5>
                
                @php($fasilitas = explode("・",$kos->fasilitas))
            ...
            <div class="rows">
                ...
                <div class="text-capt-detail mr-3 mt-4 mb-4" style="font-family: 'Lato', sans-serif;" ><i class="fa fa-bed ml-3 mr-3"></i>{{$kos->ukuran}}</div>
            </div>
            ...
            <div class="container">
                <div class="row">
                    <div class="col-md-6 mt-3 mb-3"  style="font-family: 'Poppins', sans-serif;">
                        ...
                        <!-- Mengubah string yang terpisah koma menjadi array, contoh >> ("halo,hola,holo") menjadi ["halo","hola","holo"] -->
                        @php($aturan = explode(",",$kos->aturan))
                        <ul>
                        ...
                        <div class="col-md-6 mt-3 mb-5" style="font-family: 'Poppins', sans-serif;">
                            @if(!empty(Auth::user()))
                                ...
                                @elseif(Auth::user()->level == 'pencari')
                                    ...
                                    <div>
                                        <a data-toggle="modal" data-target="#konfirmasi" class="btn btn-block btn-sm btn-outline-success">Pesan Kos</a>
                                    </div>
                                @endif
                            @else
                                <h5 style="font-family: 'Poppins', sans-serif;">Pesan Sekarang?</h5>
                                <div>
                                    ...
                                    <a href="{{url('auth/pencari/login')}}" class="btn btn-block btn-sm btn-outline-success">Pesan Kos</a>
                                </div>
                            @endif
                        </div>
                    </div>
...
<div class="modal fade" id="konfirmasi" tabindex="-1" aria-labelledby="konfirmasi" aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <form action="{{url('kos/pesan')}}" method="post">
            @csrf
